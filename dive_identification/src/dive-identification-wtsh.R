...
foreach(file = dir('dive_identification/9B_wtsh_dive_data/', full.names = TRUE), .combine = rbind) %do% {
  read.csv(file) %>%
    mutate(Begin = as.POSIXct(Begin, tz = 'UTC') + .05,
           End = as.POSIXct(End, tz = 'UTC') + .05)
} %>% 
  filter(as.POSIXlt(Begin)$year + 1900 == 2014) %>% 
  group_by(DeployID) %>%
  summarize(Dives = n()) %>% 
