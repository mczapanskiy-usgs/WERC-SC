...
post.analysis <- function() {
  valid_dives <- dir('dive_identification/5c_wtsh_dive_data_dive_plots/Dives/') %>% 
  ...
  invalid_dives <- dir('dive_identification/5c_wtsh_dive_plots/NonDives/') %>% 
    sub(pattern = '.png', replacement = '', x = ., fixed = TRUE) %>%
    strsplit('_') %>% 
    unlist %>% 
    matrix(ncol = 3, byrow = TRUE) %>% 
  ...
  dive_stats <- lapply(dir('dive_identification/4c_wtsh_dive_data/', full.names = TRUE), 
