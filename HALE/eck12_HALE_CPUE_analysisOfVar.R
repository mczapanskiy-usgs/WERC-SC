...
cpue.year.caughts2 <- mlogit.data(expanded_data.Caughts_only,
                                 ...
                                 shape="long", 
                                 chid.var="chid")
cpue_preds_models[[1]] <- mlogit(choice ~ 0 | Season + YearCts,
                           rpar=c('ratCaught:(intercept)'='n',
                                  'mongooseCaught:(intercept)'='n'),
                           R=50, halton=NA,
                           panel=TRUE,
...
cpue.trap.caughts2 <- mlogit.data(expanded_data.Caughts_only, # %>% filter(loc == "front"), 
                                 ...
                                 shape="long", 
                                 chid.var="chid")
cpue_preds_models[[2]] <- mlogit(choice ~ 0 | Season + YearCts,
...
cpue.trapyr.caughts2 <- mlogit.data(expanded_data.Caughts_only %>% 
                                   ...
                                   shape="long", 
                                   chid.var="chid")
cpue_preds_models[[3]] <- mlogit(choice ~ 0 | Season + YearCts,
                           rpar=c('ratCaught:(intercept)'='n',
                                  'mongooseCaught:(intercept)'='n'),
                           R=50, halton=NA,
                           panel=TRUE,
...
cpue.caughts2 <- mlogit.data(expanded_data.Caughts_only,
                            ...
                            shape="long", 
                            chid.var="chid")
cpue_preds_models[[4]] <- mlogit(choice ~ 0 | Season + YearCts + Trapline,
                            iterlim=1, print.level=1,
                            data=cpue.caughts2) 
# without any random effects, Season + Year = individual-specific variables 
cpue_preds_models[[5]] <- mlogit(choice ~ 0 | Season + YearCts,
                            iterlim=1, print.level=1,
                            data=cpue.caughts2)
# without any random effects, Season = individual-specific variables 
cpue_preds_models[[6]] <- mlogit(choice ~ 0 | Season,
                            iterlim=1, print.level=1,
                            data=cpue.caughts2)
# without any random effects, Year = individual-specific variables 
cpue_preds_models[[7]] <- mlogit(choice ~ 0 | YearCts,
                            iterlim=1, print.level=1,
                            data=cpue.caughts2)
## MONTH
# Year = random variable, Season + Year = individual-specific variables
...
# now compare AIC and loglik between models
varsColS <- sapply(cpue_preds_models, function(m) substr(as.character(formula(m)[3]), start = 5, stop = 1e6))
dfColS <- sapply(cpue_preds_models, function(m) attr(logLik(m), "df"))
logLikColS <- sapply(cpue_preds_models, function(m) attr(logLik(m), "null"))
aicColS <- sapply(cpue_preds_models, function(m) AIC(m))
minAicS <- min(aicColS)
deltAicColS <- aicColS - minAicS
aicWcolS <- Weights(aicColS)

# combine into one table and save output
bestCPUE_preds <- data.frame(variables = varsColS, AIC = aicColS, `delt AIC` = deltAicColS, `Weighted AIC` = aicWcolS,
                                     `Log Likelihood` = logLikColS, DF = dfColS)
write.csv(bestCPUE_preds, file = '~/WERC-SC/HALE/outputs/bestCPUE_preds_month_eck12_20171114.csv',
          row.names = FALSE)


### analyze results for model 2 (the best fit for the "caughts_only" data)
## get fitted frequencies of each event type on unique combos of Trapline, Year, & Season
myfitted <- fitted(cpue_preds_models[[2]], outcome=FALSE)
# head(myfitted)
# dim(myfitted)
...
## MONTH
myfitted_month <- fitted(cpue_preds_models[[9]], outcome=FALSE)
# head(myfitted_month)
# dim(myfitted_month)
# dim(expanded_data.Caughts_only)

## select year, season, and trapline data for the fitted values
# Copy data and thin it down to one row per chid (i.e. 33543 rows becomes 11181; because myfitted_month (above) is already thinned to 11181 rows)
fitted_cpue_month <- expanded_data.Caughts_only %>%
  select(chid, Trapline, Year, Month) %>%
  unique()
# then `cbind` the data in `fitted_cpue_month` with the fitted values in `myfitted_month`
fitted_cpue_month <- cbind(fitted_cpue_month, myfitted_month) %>%
  # thin the fitted values further (i.e. remove replicates and keep the unique combos of Trapline, Year, & Season)
  select(-chid) %>%
  unique()
