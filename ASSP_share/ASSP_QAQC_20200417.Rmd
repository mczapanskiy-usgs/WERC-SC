...
The database is an Excel file that contains (6) sheets: Banding, Banding_Data_Dictionary, CPUE, CPUE_Data_Dictionary, Mistnetting_Locations, Participant_Initials. 

This RMarkdown document is divided into two categories: Banding QAQC and CPUE QAQC. Amelia DuVall conducted QAQC on the Banding data and Emma Kelsey conducted QAQC on the CPUE metadata. 
...
individual marked as LHSP and ASSP

replaced bands, added field repalced, also added ind_code

## Capture time
```{r capturetime}
# Isolate hour as a new field. 
...
metadata_effort <- metadata %>%
                      select(session_ID, site_code, app_sunset, std_ending, lat, long)
...
NOTE: two sessions without start dates were omitted.
...
```
The graph and summary stats above show minutes calcuated for each netting session. We want to check that minutes were added correctly across multiple open/close sessions and also that minutes were added accurately across midnight - four sessions were not added correctly (the outliers to the far right). We couldn't figure out how these were miscalculated, but were able to change them manually to the correct amount of time for those four sessions for the final database version and analysis. 
NOTE: The 24 sessions without net open/closed times were omitted. 
...
```
The above graph and summary stats show the total number of mintues for the standardized session (from net open to net close or standard ending whichever came first). The standard ending is 5.3 hours (318 minutes) after sunset. Nets opened at or after sunset.  
NOTE: The 24 sessions without net open/closed times were omitted. 
...
ggplot(metadata, aes(net_dim, ASSPstd)) +
  ...
  theme_bw()
```
The graph above shows the number of ASSP caught within the standardized period in relation to net dimensions.  Number at top of each box plot = sample size. The size of the net doesn't seem to effect the number of birds caught in a specific night. 
NOTE: 27 sessions with NAs were omitted: 24 sessions did not have start and stop times, another 3 did not have any ASSP catches.
Below we will explore this effect with number of catches standardized to effort. 
...
NOTE: 141 sessions with NAs were omitted. The time difference calculation btwn catch time and sunset didn't work for 141 of the 4158 captures for this a visualization exercise.
...
ggplot(catches, aes(catchPastSS)) +
  ...
  theme_bw()
```
There doesn't appear to be any distinct pattern in timing of catches between locations, especially given the variation in sample sizes between locations.  NOTE: 141 sessions with NAs were omitted. The time difference calculation btwn catch time and sunset didn't work for 141 of the 4158 captures for this a visualization exercise.
...
NOTE: the 46 NA entires for CPUEraw were omitted.  
...
NOTE: the 47 NA entires for CPUEstd were omitted.  
...
ggplot(metadata, aes(CPUEraw, CPUEstd)) +
  ...
  theme_bw()
```
The above graph explores the correlation between CPUE and CPUE std.  Blue line = slope of 1. As expected, the correlation is often 1:1, but with variation as the number of ASSP caught and number of mistnetting minutes were both effected by the standard ending cutoff but not always in a proportional way.  Turquoise points = data that has been flagged due to inconsistencies in data entry. Sessions with high CPUE were flagged.  Nothing in the data entry suggests these entries were erroneous.
NOTE: the 47 NA entires for CPUEstd were omitted.  
...
ggplot(metadata_BP, aes(month, BPfreq_Y)) +
  geom_boxplot() +
...
NOTE: the 79 captures without broodpatch recorded were omitted.
