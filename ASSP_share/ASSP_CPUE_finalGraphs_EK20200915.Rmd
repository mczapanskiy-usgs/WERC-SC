...
catches <-  banding %>% 
  ...
  # mutate(app_sunset = as.POSIXct(app_sunset), capture_date = as.POSIXct(capture_date),
  #        catchesPastSS = duration(capture_date, app_sunset, "minutes")) %>% 
  mutate(app_sunset = as.POSIXct(app_sunset), capture_date = as.POSIXct(capture_date),
         catchPastSS = difftime(capture_date, app_sunset, tz = "America/Los_Angeles", units = "mins")) %>%
  # mutate(app_sunset = as.POSIXct(app_sunset), capture_date = as.POSIXct(capture_date),
  #        catchPastSS = minutes(app_sunset) - minutes(capture_date)) %>%  # %>% app_sunset - capture_date) %>% 
