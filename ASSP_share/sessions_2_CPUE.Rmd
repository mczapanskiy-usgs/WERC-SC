...
# Filter to ASSP spp only and remove SNRs
ASSP <- captures %>%
  filter(species == "ASSP" & recapture != "SNR") # 3815 observations
```
...
cpue <- ASSP_std %>%
  ...
  summarise(ASSP = n(), 
            ASSPstd = sum(std == "1"),
            BPct = count(!is.na(BP)), # double-check
            BP_Y = sum(assumeBreed == "Y", na.rm = TRUE), # number of birds that have a broodpatch (2-4, B)
            BP_N = sum(assumeBreed == "N", na.rm = TRUE)) %>% # number of birds that dont have a broodpatch (1, 1.5, 4.5, 5, PB, D)
  ungroup() %>%
...
BPsNA <- ASSP %>%
  filter(is.na(BP))
BPsIDS <- unique(BPsNA$session_ID)
## these are session_IDs with NA for BP scores
