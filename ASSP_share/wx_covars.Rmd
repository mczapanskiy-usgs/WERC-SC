---
title: "CHIS ASSP Weather Covariate Data"
...
This notebook documents pulling `ERA5` data for weather covariates for the Ashy Storm-Petrel catch-per-unit-effort analysis at Channel Islands National Park. 
...
# no longer maintained and may not work with new versions of R.  We can discus
...
## I used miniconda to get python to work. 
...
# load the data
dat_raw<-readRDS(here::here("WERC-SC", "ASSP_share", "ASSP_4_wx_v2.RDS"))
...
dat<-dat_raw %>% 
  ...
  mutate(Longitude360=ifelse(long<0,long+360,long),
         Latitude=lat)

## do not need to fix time zones  
...
# use_python(python = "C:/Users/Amelia DuVall/AppData/Local/Programs/Python/Python38-32/python.exe")

## I opted to use rminiconda instead. 
rminiconda::install_miniconda(version = 3, name = "my_python")
...
# years = ['1994', '1995', '2004', '2005', '2006', '2007', '2009', '2011', '2014', '2015', '2016', '2017', '2018']
years = ['1995', '2007','2017', '2018']
months = [4, 5, 6, 7, 8, 9]
...
for jj in years:
      ...
      'total_precipitation'
			...
			'C:/Users/Amelia DuVall/wx/era5_'+jj+'_N'+'{:02d}'.format(lats[1])+'_W'+'{:03d}'.format(lons[0])+'_S'+'{:02d}'.format(lats[0])+'_E'+'{:03d}'.format(lons[1])+'.nc')
```
...
files<-list.files(here::here("WERC-SC", "ASSP_share", 'ERA5'),pattern = "\\.nc$",full.names = T,recursive = T)
files<-as.character(files)

# open a connection to one of the files
...
```{r}
# hourly output
saveRDS(dat,here::here("WERC-SC", "ASSP_share", "mistnet_w_weather_hrly.rds"))
write.csv(dat,here::here("WERC-SC", "ASSP_share", "mistnet_w_weather_hrly.csv"),row.names=F)
...
saveRDS(dat_out,here::here("WERC-SC", "ASSP_share", "mistnet_w_weather.rds"))
write.csv(dat_out,here::here("WERC-SC", "ASSP_share", "mistnet_w_weather.csv"),row.names=F)
```
