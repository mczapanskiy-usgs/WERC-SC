...
```{r}
metadata <- read.csv('~/WERC-SC/ASSP_share/cpue_ScienceBase.csv') %>%
...
catches <- read.csv('~/WERC-SC/ASSP_share/captures_ScienceBase.csv') %>%
  mutate_at(c("capture_date", "release_date"), # "app_sunset", "std_ending", 
            .funs = ~as.POSIXct(., format="%Y-%m-%d %H:%M:%S")) %>%
  filter(TRUE)
...
```{r}
ggplot(metadata, aes(month, app_sunset)) +
