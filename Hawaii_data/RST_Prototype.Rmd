...
```{r static.R, eval=FALSE, cache=TRUE, include=FALSE}
speed.data <- RST.data %>%
  group_by(band) %>%
...
R <- mean.transit.spd * sample.interval / 2 / 1000 # (convert to km)
```

```{r static.res, eval=FALSE, cache=TRUE, include=FALSE}
all_tracks = data.frame()
...
for (i in 1:length(bandIDs)) {
	...
	result <- residenceCalc(subdata$x, subdata$y, subdata$time_diff, R, threshold)
	subdata = cbind(subdata, result)
	all_tracks = rbind(all_tracks, subdata)
}
```

```{r plot.static, eval=FALSE, cache=TRUE, include=FALSE}
# This function plots the track contours, color-coded by both residence time and distance:
x11()#for Windows users
plotTrackContour(all_tracks, R, 0)

# This function plots the residual values and the track contour, color-coded by sign of residual:
x11()#for Windows users
plotTrackRes(all_tracks, R, 0, time_units)
```
